...
import ProductFilter from "../components/filters/Kidsproductfilter";
import ProductCard from "../components/ProductCard";
import kidsProducts from "../Dummydata/kidsProducts"; // Import the kids dummy data
...
const KidsClothing = () => {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        // Update API endpoint to fetch kids clothing
        const res = await fetch("/api/products?category=kids");
        if (!res.ok) throw new Error("Failed to fetch from API");
        const data = await res.json();
        setProducts(data);
      } catch (err) {
        console.warn("Using dummy data for kids clothing:", err.message);
  ...
  useEffect(() => {
    let filtered = [...products]; // Start with all kids products
    ...
    <div className="flex flex-col lg:flex-row gap-6 p-4 h-[calc(100vh-80px)]">
      ...
      >
        <ProductFilter
          ...
          gender="kids" // Pass gender prop as "kids"
        />
      </div>
